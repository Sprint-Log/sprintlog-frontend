...
Resizable.initialise = function (parentId, initialSizes, resizerThickness) {
	...
	var parentWindow = new Resizable.ContentWindow(
		null,
...
Resizable.setupChildren = function (parentWindow) {
	...
	if (sizeFraction == undefined) sizeFraction = 0.5;
...
Resizable.ContentWindow = class {
	constructor(parent, width, height, div) {
		...
		if (div == null) {
			...
			if (div.id == '') div.id = 'contentWindow' + Resizable.activeContentWindows.length;
			this.divId = div.id;
	...
	findChildWindowElements() {
		...
		//Find left child
		if (
			document.querySelectorAll(`#${this.divId} > .${Resizable.Classes.WINDOW_LEFT}`).length > 0
		) {
			child1 = document.querySelectorAll(`#${this.divId} > .${Resizable.Classes.WINDOW_LEFT}`)[0];
			if (
				document.querySelectorAll(`#${this.divId} > .${Resizable.Classes.WINDOW_LEFT}`).length > 0
			) {
				child2 = document.querySelectorAll(
					`#${this.divId} > .${Resizable.Classes.WINDOW_RIGHT}`
				)[0];
			} else {
		...
		if (document.querySelectorAll(`#${this.divId} > .${Resizable.Classes.WINDOW_TOP}`).length > 0) {
			if (child1 != undefined) {
				...
				if (
					document.querySelectorAll(`#${this.divId} > .${Resizable.Classes.WINDOW_BOTTOM}`).length >
					0
				) {
					child2 = document.querySelectorAll(
						`#${this.divId} > .${Resizable.Classes.WINDOW_BOTTOM}`
					)[0];
				} else {
	...
	getSibling() {
		...
		if (this.parent.children[0] == this) return this.parent.children[1];
		else return this.parent.children[0];
	}

	childrenResize() {
		if (this.children.length == 0) return; //Content window has no children
		...
		if (this.isSplitHorizontally) {
			...
			this.children[1].getDiv().style.left =
				parseInt(this.children[0].getDiv().style.width) + this.childResizer.lineThickness + 'px';
			...
			this.children[1].getDiv().style.top =
				parseInt(this.children[0].getDiv().style.height) + this.childResizer.lineThickness + 'px';
		}
	...
	toString() {
		return `divId = ${this.divId}, parent = ${this.parent.getDivId()}, width = ${
			this.width
		}, height = ${this.height}`;
	}
	...
	changeSize(width, height) {
		...
		if (this.parent != null) {
			if (
				...
				this.parent.isSplitHorizontally
			) {
				width = this.parent.width - this.getSibling().minWidth;
		...
		if (this.parent == null) {
			...
			if (height > window.height) height = window.innerHeight;
		} else {
	...
	repositionChildResizer() {
		if (this.childResizer != null) this.childResizer.reposition();
	}
	...
	splitHorizontally(leftWindowSizeFraction, leftDiv, rightDiv) {
		...
		var w2 = new Resizable.ContentWindow(
			this,
			this.width - leftWidth - this.childResizerThickness / 2,
			this.height,
			rightDiv
		);
		...
		this.childResizer.getDiv().style.left =
			Math.round(leftWidth - this.childResizerThickness / 2) + 'px';
	...
	splitVertically(topWindowSizeFraction, topDiv, bottomDiv) {
		...
		var w1 = new Resizable.ContentWindow(
			this,
			this.width,
			topHeight - this.childResizerThickness / 2,
			topDiv
		);
		var w2 = new Resizable.ContentWindow(
			this,
			this.width,
			this.height - topHeight - this.childResizerThickness / 2,
			bottomDiv
		);
		...
		this.childResizer.getDiv().style.top =
			Math.round(topHeight - this.childResizerThickness / 2) + 'px';
...
Resizable.Resizer = class {
	...
	resize(e) {
		...
		if (Resizable.currentResizer.isHorizontal) {
			...
			Resizable.currentResizer.getDiv().style.left =
				Resizable.currentResizer.leftWindow.getDiv().style.width;
			Resizable.currentResizer.rightWindow.resize(
				Resizable.Sides.LEFT,
				parseInt(Resizable.currentResizer.getDiv().style.left)
			);
		} else {
			...
			Resizable.currentResizer.getDiv().style.top =
				Resizable.currentResizer.topWindow.getDiv().style.height;
			Resizable.currentResizer.bottomWindow.resize(
				Resizable.Sides.TOP,
				parseInt(Resizable.currentResizer.getDiv().style.top)
			);
		}
	}
	...
	delete() {
		for (var i = 0; i < Resizable.activeResizers.length; i++) {
			if (Resizable.activeResizers[i] == this) Resizable.activeResizers.splice(i, 1);
		}
