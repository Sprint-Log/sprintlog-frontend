<script lang="ts">
	...
	onMount(() => {
		view = new EditorView({
			doc: description,
			extensions: [codeMirrorSetup, theme.of(oneDark), markdown(), keymap.of(saveKeyMap)],
			parent: editor
		});
...
<div class="min-h-[10rem] max-h-30 rounded-container-token overflow-y-scroll py-1 px-2" bind:this={editor}  />
